...
const listAvailablePorts = async () => {
    try {
        ...
        const comPorts = ports.filter(port => port.path && (
            port.path.startsWith('COM') || // Windows
            port.path.startsWith('/dev/tty') // Linux/Mac
        ));

        if (comPorts.length === 0) {
            console.log('⚠️ No available COM/serial ports found');
            return null;
        }

        console.log('📡 Available ports:', comPorts.map(p => p.path).join(', '));
...
const initRFIDReader = async () => {
    try {
        ...
        if (!portPath) {
            console.log('⏳ No RFID reader found, will retry in 5 seconds...');
            return false;
        }

        console.log('🔍 Attempting to connect to RFID reader on:', portPath);
        ...
        console.error('❌ RFID reader initialization error:', err.message);
        return false;
    }
};
